...
def evaluate(
    ...
    with torch.inference_mode():
        ...
        for set_name, batch, global_batch_size in eval_loader:
            ...
            # Per-example correctness (skip padded entries)
            labels = batch["labels"]
            mask = (labels != IGNORE_LABEL_ID)
            per_token_correct = (preds["preds"] == labels) & mask
            seq_is_correct = per_token_correct.sum(-1) == mask.sum(-1)
            valid_examples = (batch["puzzle_identifiers"] != eval_metadata.blank_identifier_id) & (mask.sum(-1) > 0)

            if valid_examples.any():
                valid_indices = torch.nonzero(valid_examples, as_tuple=False).squeeze(-1)
                per_example_results["set_name"].extend([set_name] * valid_indices.numel())
                per_example_results["puzzle_identifier"].extend(batch["puzzle_identifiers"][valid_indices].tolist())
                per_example_results["exact_correct"].extend(seq_is_correct[valid_indices].tolist())
                per_example_results["token_correct"].extend(per_token_correct.sum(-1)[valid_indices].tolist())
                per_example_results["token_total"].extend(mask.sum(-1)[valid_indices].tolist())
                per_example_results["steps"].extend(carry.steps[valid_indices].tolist())
        ...
        # Save per-example correctness
        if config.checkpoint_path is not None and len(per_example_results["set_name"]) > 0:
            os.makedirs(config.checkpoint_path, exist_ok=True)
            per_example_path = os.path.join(config.checkpoint_path, "per_example_correct.pt")
            torch.save(per_example_results, per_example_path)
            print(f"Per-example correctness saved to: {per_example_path}")
