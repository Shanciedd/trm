...
class EvalConfig(pydantic.BaseModel):
    ...
    # Extras
    seed: int = 0
    eval_save_outputs: List[str] = ["preds"]
...
def save_code_and_config(config: EvalConfig):
    ...
    # Dump config as yaml
    config_file = os.path.join(config.checkpoint_path, "eval_config_ablation.yaml")
    with open(config_file, "wt") as f:
        yaml.dump(config.model_dump(), f)


@hydra.main(config_path="config", config_name="cfg_eval_ablation", version_base=None)
